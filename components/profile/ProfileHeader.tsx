...
export function ProfileHeader({
  ...
  const { walletData, loading, error: walletError } = useProfileWalletAddresses(drawerOpen ? fid : undefined);
      ...
      <div className="flex-1 min-w-0">
        <Drawer open={drawerOpen} onOpenChange={handleOpenChange}>
          ...
          <DrawerContent className="max-w-sm mx-auto w-full p-4 rounded-t-2xl">
            <DrawerHeader>
              <DrawerTitle className="text-lg font-semibold mb-2">Wallet Addresses</DrawerTitle>
              <DrawerDescription className="sr-only">
                List of all wallet addresses and ENS names associated with your account. Tap the copy icon to copy an
                address.
              </DrawerDescription>
            </DrawerHeader>
            <div className="px-2 pb-4 flex flex-col gap-3">
              {!fid ? (
                ...
                walletData.addresses.map((address: string) => (
                  <div key={address} className="flex items-center gap-3 h-14 px-3 bg-muted rounded-xl">
                    ...
                    <svg width="20" height="20" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <path d="M16 23.6L16.1 23.8V28.7L16 29L7.1 18.7L16 23.6Z" fill="#232323" />
        ...
        <div className="mt-1 flex flex-col gap-0.5">
          <span className="text-muted-foreground text-sm">{followers ?? "—"} total followers</span>
        </div>
      </div>
